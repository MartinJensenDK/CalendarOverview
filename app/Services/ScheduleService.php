...
/**
 * Free/busy information for many users, cached in schedule_items and refreshed
 * through POST /me/calendar/getSchedule (delegated Calendars.Read).
 */
class ScheduleService
{
     ...
     * @return array{items: array<string, list<array>>, errors: array<string, string>, fetched_at: string}
     */
    public function itemsFor(User $actor, Collection $users, CarbonImmutable $from, CarbonImmutable $to, string $tz, bool $force = false): array
    {
        ...
        return ['items' => $items, 'errors' => $errors, 'fetched_at' => Carbon::now()->toIso8601String()];
    }
    ...
    private function store(array $parsed, CarbonImmutable $from, CarbonImmutable $to, string $tz): void
    {
        if ($parsed === []) {
            return;
        }
        ...
        DB::transaction(function () use ($parsed, $from, $to, $now, $days) {
