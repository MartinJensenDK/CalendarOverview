...
export default {
  ...
  template: `
    <modal :title="t('Settings')" width="560px" @close="closeModal">
      ...
      <label class="switch block" style="margin-bottom:14px"><input type="checkbox" :checked="store.prefs.vacation_show_weekends" @change="set({ vacation_show_weekends: $event.target.checked })"><span class="track"></span>{{ t('Show weekends') }}</label>
      <label class="switch block" style="margin-bottom:14px"><input type="checkbox" :checked="store.prefs.vacation_week_numbers" @change="set({ vacation_week_numbers: $event.target.checked })"><span class="track"></span>{{ t('Show week numbers') }}</label>
